...
NEGATIONS = {
    "not", "no", "never", "without", "don't", "do not", "must not",
    "kein", "keine", "keinen", "nicht", "niemals", "ohne", "darf nicht",
}


def _preservation_check(name: str, items: Iterable[str], compressed: str) -> SafetyCheck:
    expected = sorted({item for item in items if item}, key=lambda value: (-len(value), value))
    ...
    return SafetyCheck(name, relevant, not missing, expected, preserved, missing)
...
def extract_required_items(text: str) -> dict[str, set[str]]:
    return {
        "Negations": extract_negations(text),
        "CLI flags": extract_cli_flags(text),
        "File paths": extract_file_paths(text),
        "JSON keys": extract_json_keys(text),
        "Versions": extract_versions(text),
        "Code symbols": extract_code_symbols(text),
    }


def run_safety_checks(original: str, compressed: str) -> list[SafetyCheck]:
    return [
        _preservation_check(name, items, compressed)
        for name, items in extract_required_items(original).items()
    ]
